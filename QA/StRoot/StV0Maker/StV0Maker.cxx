...
void StV0Maker::initParam() {
    ...
    //*************************************
    double tempValue = 0.;
    ifstream pionInputStream("/star/u/jhai/dedx/");
    ifstream protonInputStream("/star/u/jhai/dedx/");
    ifstream deutronInputStream("/star/u/jhai/dedx/");
    ifstream tritonInputStream("/star/u/jhai/dedx/");
    ifstream helium3InputStream("/star/u/jhai/dedx/");
    ifstream helium4InputStream("/star/u/jhai/dedx/");
    for (int i = 0; i != 11901; ++i) {
        pionInputStream >> tempValue >> pionTheory[i];
        protonInputStream >> tempValue >> protonTheory[i];
        deutronInputStream >> tempValue >> deutronTheory[i];
        tritonInputStream >> tempValue >> tritonTheory[i];
        helium3InputStream >> tempValue >> helium3Theory[i];
        helium4InputStream >> tempValue >> helium4Theory[i];
        pionTheory[i] *= 1.e-6;
        protonTheory[i] *= 1.e-6;
        deutronTheory[i] *= 1.e-6;
        tritonTheory[i] *= 1.e-6;
        helium3Theory[i] *= 1.e-6;
        helium4Theory[i] *= 1.e-6;
    }
    pionInputStream.close();
    protonInputStream.close();
    deutronInputStream.close();
    tritonInputStream.close();
    helium3InputStream.close();
    helium4InputStream.close();
    //*************************************
    // QA (end)
    //*************************************
    return;
...
void StV0Maker::initHisto() {
    ...
    hVertexZ = new TH1F("VertexZ", "Event Vertex Z Position", nbins * 20, 50.0, 250.0);
    hisVPDZ = new TH1F("VPDZ", "VPDZ", nbins * 20, 50.0, 250.0);
...
void StV0Maker::initTree() {
    ...
    mV0Tree->Branch("numVertex", &mV0Dst.numVertex, "numVertex/I");
...
Int_t StV0Maker::Init() {
    ...
    if (mHistogramOutputFileName == "") {
        std::cerr << "StV0Maker: Please specify the histrogram output file !!!!!" << std::endl;
    } else {
    ...
    if (mV0TreeOutputFileName == "") {
        std::cerr << "StV0Maker: The V0 tree output file is not specified! output is smeared!" << std::endl;
    } else {
        ...
        // QA(begin)
        //*****************************
        QATreeOutputFile = new TFile(QATreeOutputFileName, "recreate");
        initQA();
        //*****************************
        // QA (end)
        //*****************************
...
Int_t StV0Maker::Make() {
    ...
    //*****************************
    objectQA.pionNum = 0;
    objectQA.protonNum = 0;
    objectQA.deutronNum = 0;
    objectQA.tritonNum = 0;
    objectQA.helium3Num = 0;
    objectQA.helium4Num = 0;
    //*****************************
    // QA (end)
    //*****************************
    mPassEventCut = false;
    ...
    hNPrimVertex->Fill(muDst->numberOfPrimaryVertices());
    mV0Dst.numVertex = static_cast<int>(muDst->numberOfPrimaryVertices());

    StThreeVectorF primaryVertex = muEvent->primaryVertexPosition();
    hVertexZ->Fill(primaryVertex.z());
    if ((fabs(primaryVertex.x()) < 1e-5f) && (fabs(primaryVertex.y()) < 1e-5f) && (fabs(primaryVertex.z()) < 1e-5f)) {
        return kStOK;
    }
    if (primaryVertex.z() > 212 || primaryVertex.z() < 210) {
        return kStOK;
    }
    /*if (primaryVertex.perp() > 2) {
        return kStOK;
    }*/
    mV0Dst.primvertexX = primaryVertex.x();
    mV0Dst.primvertexY = primaryVertex.y();
    mV0Dst.primvertexZ = primaryVertex.z();
    objectQA.primeryVertexX = static_cast<double>(primaryVertex.x());
    objectQA.primeryVertexY = static_cast<double>(primaryVertex.y());
    objectQA.primeryVertexZ = static_cast<double>(primaryVertex.z());

    //	Float_t temprefMult = muEvent->refMult();
    Float_t temprefMult = muEvent->grefmult();
    hNRefMult->Fill(temprefMult);
    objectQA.referenceMulti = muEvent->grefmult();
    ...
    hSelectNRefMult->Fill(temprefMult);

    mV0Dst.eventid = muEvent->eventId();
    mV0Dst.runid = muEvent->runInfo().runId();
    objectQA.eventId = muEvent->eventId();
    objectQA.runId = muEvent->runId();
    ...
    hisVPDZ->Fill(mvpdVz);
    mV0Dst.vpdVz = mvpdVz;
    hVertexZdiff->Fill(mvpdVz - primaryVertex.z());
    ...
    int pionCount = 0, protonCount = 0, deutronCount = 0, tritonCount = 0, helium3Count = 0, helium4Count = 0;
    ...
    while ((track = (StMuTrack*)GetTracks.Next())) {
        ...
        /*! do scan periods.NOTE:the default is false.this is not necessary for tracks with pt>0.15GeV/c  */
        double pathlength = helix.pathLength(pv, false);
        ...
        if (abs(charge) != 1) {
            continue;
        }  // Let's comment out this line for checking
        if (fabs(track->eta()) > 2.5) {
            continue;
        }  // 1.0 to 1.5 to include more He3, pion should keep (-1.,1.0)
        ...
        if (p.perp() < cutPtGrEq) {
            continue;
        }  // should be larger. like 0.15 or 0.2
        ...
        if (tofbeta > 0.0) {
            hInvBetaP->Fill(p.mag() * charge, 1.0 / tofbeta);
        }
        hMassP->Fill(p.mag() * charge, tofmass);
        hDcaP->Fill(p.mag() * charge, dca.mag());

        // He3 charge = 2. build in mom p/q,should be corrected here!
        Double_t dau1Z = TMath::Log(dedx / dedx_dau1_th[index_for_p(abs(mCharge1) * p.mag())]);
        // He3 charge = 2. build in mom p/q,should be corrected here!
        Double_t dau2Z = TMath::Log(dedx / dedx_dau2_th[index_for_p(abs(mCharge2) * p.mag())]);
        // deutron Z
        double deutronZ = TMath::Log(dedx / deutronTheory[index_for_p(abs(DeutronCharge) * p.mag())]);
        // triton Z
        double tritonZ = TMath::Log(dedx / tritonTheory[index_for_p(abs(TritonCharge) * p.mag())]);
        // helium3 Z
        double helium3Z = TMath::Log(dedx / helium3Theory[index_for_p(abs(Helium3Charge) * p.mag())]);
        // helium4 Z
        double helium4Z = TMath::Log(dedx / helium4Theory[index_for_p(abs(Helium4Charge) * p.mag())]);
        ...
        if (charge == PionCharge / abs(PionCharge) && fabs(nsigmapion) < 3.0) {
            if (dca.mag() < 0.0) {
                continue;
            }

            StThreeVectorD momentum = fabs(PionCharge) * p;
            objectQA.pionId[pionCount] = trackid;
            objectQA.pionDca[pionCount] = dca.mag();
            objectQA.pionMass[pionCount] = tofmass;
            objectQA.pionEta[pionCount] = tracketa;
            objectQA.pionRapidity[pionCount] = 0;
            objectQA.pionNumHitsFit[pionCount] = nHitsFit;
            objectQA.pionNumHitsdEdx[pionCount] = nHitsDedx;
            objectQA.pionNumHitsPossible[pionCount] = track->nHitsPoss();
            objectQA.pionNSigma[pionCount] = nsigmapion;
            objectQA.pionPx[pionCount] = momentum.x();
            objectQA.pionPy[pionCount] = momentum.y();
            objectQA.pionPz[pionCount] = momentum.z();
            objectQA.pionPt[pionCount] = momentum.perp();
            objectQA.piondEdx[pionCount] = dedx;
            objectQA.pionZ[pionCount] = 0;
            objectQA.pionBeta[pionCount] = tofbeta;
            ++pionCount;
        }

        if (charge == ProtonCharge / abs(ProtonCharge) && fabs(nsigmaproton) < 3.0) {
            if (dca.mag() < 0.0) {
                continue;
            }

            StThreeVectorD momentum = fabs(ProtonCharge) * p;
            objectQA.protonId[protonCount] = trackid;
            objectQA.protonDca[protonCount] = dca.mag();
            objectQA.protonMass[protonCount] = tofmass;
            objectQA.protonEta[protonCount] = tracketa;
            objectQA.protonRapidity[protonCount] = 0;
            objectQA.protonNumHitsFit[protonCount] = nHitsFit;
            objectQA.protonNumHitsdEdx[protonCount] = nHitsDedx;
            objectQA.protonNumHitsPossible[protonCount] = track->nHitsPoss();
            objectQA.protonNSigma[protonCount] = nsigmaproton;
            objectQA.protonPx[protonCount] = momentum.x();
            objectQA.protonPy[protonCount] = momentum.y();
            objectQA.protonPz[protonCount] = momentum.z();
            objectQA.protonPt[protonCount] = momentum.perp();
            objectQA.protondEdx[protonCount] = dedx;
            objectQA.protonZ[protonCount] = 0;
            objectQA.protonBeta[protonCount] = tofbeta;
            ++protonCount;
        }

        if (charge == DeutronCharge / abs(DeutronCharge) && (deutronZ > -0.5 && deutronZ < 0.5)) {
            if (dca.mag() < 0.0) {
                continue;
            }
            StThreeVectorD momentum = fabs(DeutronCharge) * p;
            objectQA.deutronId[deutronCount] = trackid;
            objectQA.deutronDca[deutronCount] = dca.mag();
            objectQA.deutronMass[deutronCount] = tofmass;
            objectQA.deutronEta[deutronCount] = tracketa;
            objectQA.deutronRapidity[deutronCount] = 0;
            objectQA.deutronNumHitsFit[deutronCount] = nHitsFit;
            objectQA.deutronNumHitsdEdx[deutronCount] = nHitsDedx;
            objectQA.deutronNumHitsPossible[deutronCount] = track->nHitsPoss();
            objectQA.deutronNSigma[deutronCount] = 0.0;
            objectQA.deutronPx[deutronCount] = momentum.x();
            objectQA.deutronPy[deutronCount] = momentum.y();
            objectQA.deutronPz[deutronCount] = momentum.z();
            objectQA.deutronPt[deutronCount] = momentum.perp();
            objectQA.deutrondEdx[deutronCount] = dedx;
            objectQA.deutronZ[deutronCount] = deutronZ;
            objectQA.deutronBeta[deutronCount] = tofbeta;
            ++deutronCount;
        }

        if (charge == TritonCharge / abs(TritonCharge) && (tritonZ > -0.5 && tritonZ < 0.5)) {
            if (dca.mag() < 0.0) {
                continue;
            }
            StThreeVectorD momentum = fabs(TritonCharge) * p;
            objectQA.tritonId[tritonCount] = trackid;
            objectQA.tritonDca[tritonCount] = dca.mag();
            objectQA.tritonMass[tritonCount] = tofmass;
            objectQA.tritonEta[tritonCount] = tracketa;
            objectQA.tritonRapidity[tritonCount] = 0;
            objectQA.tritonNumHitsFit[tritonCount] = nHitsFit;
            objectQA.tritonNumHitsdEdx[tritonCount] = nHitsDedx;
            objectQA.tritonNumHitsPossible[tritonCount] = track->nHitsPoss();
            objectQA.tritonNSigma[tritonCount] = 0.0;
            objectQA.tritonPx[tritonCount] = momentum.x();
            objectQA.tritonPy[tritonCount] = momentum.y();
            objectQA.tritonPz[tritonCount] = momentum.z();
            objectQA.tritonPt[tritonCount] = momentum.perp();
            objectQA.tritondEdx[tritonCount] = dedx;
            objectQA.tritonZ[tritonCount] = tritonZ;
            objectQA.tritonBeta[tritonCount] = tofbeta;
            ++tritonCount;
        }

        if (charge == Helium3Charge / abs(Helium3Charge) && (helium3Z > -0.5 && helium3Z < 0.5)) {
            if (dca.mag() < 0.0) {
                continue;
            }
            StThreeVectorD momentum = fabs(Helium3Charge) * p;
            objectQA.helium3Id[helium3Count] = trackid;
            objectQA.helium3Dca[helium3Count] = dca.mag();
            objectQA.helium3Mass[helium3Count] = tofmass;
            objectQA.helium3Eta[helium3Count] = tracketa;
            objectQA.helium3Rapidity[helium3Count] = 0;
            objectQA.helium3NumHitsFit[helium3Count] = nHitsFit;
            objectQA.helium3NumHitsdEdx[helium3Count] = nHitsDedx;
            objectQA.helium3NumHitsPossible[helium3Count] = track->nHitsPoss();
            objectQA.helium3NSigma[helium3Count] = 0.0;
            objectQA.helium3Px[helium3Count] = momentum.x();
            objectQA.helium3Py[helium3Count] = momentum.y();
            objectQA.helium3Pz[helium3Count] = momentum.z();
            objectQA.helium3Pt[helium3Count] = momentum.perp();
            objectQA.helium3dEdx[helium3Count] = dedx;
            objectQA.helium3Z[helium3Count] = helium3Z;
            objectQA.helium3Beta[helium3Count] = tofbeta;
            ++helium3Count;
        }

        if (charge == helium4Charge / abs(helium4Charge) && (helium4Z > -0.5 && helium4Z < 0.5)) {
            if (dca.mag() < 0.0) {
                continue;
            }
            StThreeVectorD momentum = fabs(helium4Charge) * p;
            objectQA.helium4Id[helium4Count] = trackid;
            objectQA.helium4Dca[helium4Count] = dca.mag();
            objectQA.helium4Mass[helium4Count] = tofmass;
            objectQA.helium4Eta[helium4Count] = tracketa;
            objectQA.helium4Rapidity[helium4Count] = 0;
            objectQA.helium4NumHitsFit[helium4Count] = nHitsFit;
            objectQA.helium4NumHitsdEdx[helium4Count] = nHitsDedx;
            objectQA.helium4NumHitsPossible[helium4Count] = track->nHitsPoss();
            objectQA.helium4NSigma[helium4Count] = 0.0;
            objectQA.helium4Px[helium4Count] = momentum.x();
            objectQA.helium4Py[helium4Count] = momentum.y();
            objectQA.helium4Pz[helium4Count] = momentum.z();
            objectQA.helium4Pt[helium4Count] = momentum.perp();
            objectQA.helium4dEdx[helium4Count] = dedx;
            objectQA.helium4Z[helium4Count] = helium4Z;
            objectQA.helium4Beta[helium4Count] = tofbeta;
            ++helium4Count;
        }
    }
    objectQA.pionNum = pionCount;
    objectQA.protonNum = protonCount;
    objectQA.deutronNum = deutronCount;
    objectQA.tritonNum = tritonCount;
    objectQA.helium3Num = helium3Count;
    objectQA.helium4Num = helium4Count;
    QATree->Fill();
    ...
    for (unsigned int i = 0; i < mDauVec1.size(); i++) {
        ...
        for (unsigned int j = 0; j < mDauVec2.size(); j++) {
            ...
            /* if (fabs(v0mass - mMassV0) > cutV0MassWidthLeEq) {
                continue;
            }*/
            ...
            if (v0decaylength < cutV0DecLenGrEq) {
                continue;
            }
