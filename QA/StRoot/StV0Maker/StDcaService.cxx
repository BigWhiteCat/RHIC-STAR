		       ...
		       const StThreeVectorF& pv, StThreeVectorF& xv0, StThreeVectorF& op1, StThreeVectorF& op2)
{
    ...
    d_p_pi
	= sqrt((pion->Xc - proton->Xc) * (pion->Xc - proton->Xc) + (pion->Yc - proton->Yc) * (pion->Yc - proton->Yc));
    ...
    if (d_p_pi >= (r_p + r_pi)
	|| d_p_pi
	   <= fabs(r_p - r_pi)) { // XZHU: it will be rare that two tracks with d_p_pi == r_p+r_pi come from a single
				  // particle decay. that means their momenta should be in the same direction or in
				  // opposite direction. it is normal for photon conversion to have two daughter
				  // electrons in the same direction. But it will be rare for two v0 decay daughters in
				  // the very opposite direction (that means the pt of v0 is 0). FIXME: please notice
				  // the hard cut 2.0cm above, you might change it for some purposes.
	fi_root1_p = 0.;
	fi_root2_p = 0.;
	if (d_p_pi >= (r_p + r_pi)) {
	    fi_root1_pi = PI;
    ...
    fi_root1_p = fi_root1_p - floor(fi_root1_p / 2 / PI) * 2
			      * PI; // XZHU: try floor() here, instead of (int). since a wide range of period is tried
				    // below, the result should be the same. in fact, fi_root1_p should always be less
				    // than 0 (0,-2pi for 1 period), since we are looking backward for the position
				    // where the helix originates. ADDED: it is not so simple. we should keep the code
				    // as it is.
	...
	< R2_TPC) { // XZHU: if the overlapping position is close (25 cm) to the outer layer of TPC. Even if the dca
		    // between two tracks is small, it is not interesting to us anymore. We find the right periods here,
		    // which is not necessary for particles with pt > 0.15GeV/c. in recHyperon, the period finding is
		    // not used. ADDED: lots of daughters has small pt. we do not understand how multiple periods tracks
		    // are recorded (split to multiple tracks or not?). if we scan periods, we could have multiple
		    // signals for a single lambda!!! hope this effect can be simulated in embedding analysis.
	rec_zij = 1000.;
	n1 = -((int)(fabs((proton->Zc - pv.z()) / proton->Vz / 2. / PI)) + 1);
	n2 = proton->Flag; // XZHU: it is meanlingless to go forward in a helix to get the dca point where the particle
			   // (or helix) should orginates from! so i suggest to decrease this flag to 0, since
			   // fi_root1_pi is [-2pi,2pi]. ADDED: it is not so simple due to the TPC track momentum
...
void dcaPToPi(float* fi_p, float* fi_pi, StTrackHelix* proton, StTrackHelix* pion, float* d_root, float* v0, float alfa)
{
	...
	while (fabs(ddfi) > minddfi) {
		...
		do {
		    ...
		    double dfn = -dtx2 * dtx2 + (tx1 - tx2) * ddtx2 - dty2 * dty2 + (ty1 - ty2) * ddty2 - dtz2 * dtz2
				 + (tz1 - tz2) * ddtz2;
    ...
    old_d = sqrt((oldx1 - oldx2) * (oldx1 - oldx2) + (oldy1 - oldy2) * (oldy1 - oldy2) + (oldz1 - oldz2)
											 * (oldz1 - oldz2));
		       ...
		       double magnet, StThreeVectorF& xxi, StThreeVectorF& op)
{
    ...
    // xroot and yroot are the coordinates of the crossing points or the closest point between the pion circle and
    // lambda line ( in 2D ), it is still on the lambda line
    // dmax is the distance from the center of the pion circle to the Lambda
    if (dmax >= (r + 0.5))
	return 999.0;
    float newx, newy, newz, sign0 = -1, dsave = 999.0;
    if (dmax > r) { // no 2d crossing points but not too far either, then the newx and newy are the coordinates of the
		    // 2D closest point to the Lambda line but still on the circle. //XZHU: should be rare, since it
	...
	if (((newx - x) * pxK + (newy - y) * pyK) > 0)
	    sign0 = 1.0;
	newz = z + sign0 * r * 2 * asin(sqrt((newx - x) * (newx - x) + (newy - y) * (newy - y)) / 2.0 / r) * pzK
	...
	dsave = sqrt((newzL - newz) * (newzL - newz)
		     + (dmax - r) * (dmax - r)); // XZHU: fixed! in this case, we can not use the straight line
						 // approximation below. because these two lines always cross each other
						 // at somewhere far far away. therefore, you will have unphysical '0'
    ...
    } else { //	 two crssoing points, fa, fb, fc  just intermidiate numbers in calculation without any geometric
	     // meaning. newx, new y has the same meaning in case 1
	...
	if ((newx1 - x) * pxK + (newy1 - y) * pyK > 0)
	    sign0 = 1.0;
	float newz1 = z + sign0 * 2.0 * r * asin(da1 / 2.0) * pzK
			  / sqrt(pxK * pxK + pyK * pyK); // XZHU: only work in one period. for sign0=1, it is weird to
							 // check the orign point in the forward direction
	...
	// JUne 19, 2003   here calculate the distance between two straigt line, ( helix pion is approximated as
	// straight line at the new position calculated above. lambda is always a streight line.
	float aK = pxK / pK;
	float bK = pyK / pK;
	float cK = pzK / pK;
	float aL = pxL / pL;
	...
	// if(tanK==0)
	// cout<<min_d<<" "<<fabs(dsave)<<" "<<(cK*t1-cL*t2-dsave)<<" "<<(bK*t1-bL*t2)<<" "<<(aK*t1-aL*t2)<<" "<<t1<<"
	// "<<t2<<" "<<aK<<" "<<bK<<" "<<cK<<" "<<aL<<" "<<bL<<" "<<cL<<" "<<dmax<<" "<<r<<"
