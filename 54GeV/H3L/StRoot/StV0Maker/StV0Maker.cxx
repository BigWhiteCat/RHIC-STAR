...
ClassImp(StV0Maker)  // Macro for CINT compatibility
    ...
    StV0Maker::StV0Maker(StPicoDstMaker* maker, const char* name)
    : StMaker(name) {
...
Int_t StV0Maker::Make() {
    ...
    for (int i = 0; i < nTracks; i++) {
        ...
        if (abs(nHitsFit) <= cutNHitsGr)
            continue;
        if (abs(nHitsDedx) <= cutNHitsDedxGr)
            continue;
        if (abs(charge) != 1)
        ...
        if (Float_t(abs(nHitsFit)) / Float_t(abs(nHitsMax)) < 0.52)
            continue;
        if (Float_t(abs(nHitsFit)) / Float_t(abs(nHitsMax)) > 1.02)
            continue;
        if (p.perp() < cutPtGrEq)
